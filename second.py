...
def need_to_find_inn(path: str, table_height: int):
    workbook = openpyxl.load_workbook(path)
    worksheet = workbook[SECOND_SHEET]
    return str(worksheet['A' + str(table_height)].value) in ("1", "True")
...
def main() -> None:
    ...
    # Начало цикла
    while True:
        if table_height > len(companies_inn) - 1:
            print(f'{table_height} row: OK')
            return
        inn = companies_inn[table_height - 2]

        if not need_to_find_inn(xlsx_load_file, table_height):
        ...
        while attempts <= len(api_keys):
            ...
            if not err:
                ...
                cur_key_valid = write_data_to_xlsx(BASE_DIR, file_name, data, inn, table_height)
                table_height += 1
                successful = True
...
def write_data_to_xlsx(base_dir: str, file_name: str, data: list[dict], inn: str, table_height: str) -> bool:
...
def main() -> None:
    ...
    while len(api_keys) != 0:
        ...
        if not successful:
            print(f'{table_height}: Ignore company with the INN: "{inn}", data is corrupted.')
            table_height += 1

    if len(api_keys) == 0:
        print(f'{table_height} all keys are fully used.')
...
def write_data_to_xlsx(base_dir: str, file_name: str, data: list[dict], inn: str, table_height: str) -> bool:
    ...
    print(f'{table_height}: write data to xlsx file with inn: "{inn}"')
